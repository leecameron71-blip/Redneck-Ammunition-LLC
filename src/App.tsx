...
const ProductPage = ({ product, onAddToCart }: { product: Product, onAddToCart: () => void }) => {
        ...
        <div className="flex flex-col">
          ...
          <div className="space-y-6 mb-10">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <button 
                onClick={() => {
                  if (product.checkoutUrl && product.checkoutUrl !== "https://checkout.square.site/buy/PLACEHOLDER_LINK_HERE") {
                    window.open(product.checkoutUrl, '_blank');
                  } else {
                    alert("Square checkout link not configured for this product.");
                  }
                }}
                className="tactical-button tactical-button-secondary"
              >
                Buy Now
              </button>
            </div>
    ...
    <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-12">
      ...
      <div>
        ...
        <ul className="space-y-4 text-sm text-tactical-accent/60">
          ...
          <li><button onClick={onAdminClick} className="hover:text-tactical-red">Admin Panel</button></li>
...
export default function App() {
  ...
  // Load custom checkout URLs from localStorage
  const [customLinks, setCustomLinks] = useState<Record<string, string>>(() => {
    const saved = localStorage.getItem('redneck_checkout_links');
  ...
  const updateLink = (id: string, url: string) => {
    const newLinks = { ...customLinks, [id]: url };
    setCustomLinks(newLinks);
    localStorage.setItem('redneck_checkout_links', JSON.stringify(newLinks));
  };

  // Merge default products with custom links
  const productsWithLinks = PRODUCTS.map(p => ({
    ...p,
    checkoutUrl: customLinks[p.id] || p.checkoutUrl
  }));

  const activeProduct = productsWithLinks.find(p => p.id === selectedProductId) || productsWithLinks[0];
        ...
        {isAdminOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative glass-panel w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl"
            >
              <div className="p-6 border-b border-white/10 flex justify-between items-center bg-tactical-red">
                <div className="flex items-center space-x-3">
                  <Settings className="w-6 h-6 text-white" />
                  <h2 className="text-xl font-display font-bold text-white uppercase tracking-widest">Configure Square Links</h2>
                </div>
                <button onClick={() => setIsAdminOpen(false)} className="text-white hover:rotate-90 transition-transform"><X /></button>
              </div>
              
              <div className="p-8 overflow-y-auto space-y-8 bg-tactical-gray">
                <div className="p-4 border-l-2 border-tactical-red bg-white/5 text-sm text-tactical-accent/70 leading-relaxed">
                  <p className="font-bold text-tactical-red uppercase mb-1 tracking-tighter">Instructions:</p>
                  1. Login to your Square Dashboard.<br />
                  2. Search for <strong>"Online Checkout"</strong>.<br />
                  3. Create a <strong>"Collect Payment"</strong> link for each item.<br />
                  4. Paste the URLs below. These will persist on your device.
                </div>
                ...
                <div className="space-y-6">
                  {productsWithLinks.map(p => (
                    <div key={p.id} className="space-y-2">
                      <div className="flex justify-between items-center">
                        <label className="text-xs font-bold uppercase tracking-widest text-tactical-accent/40">{p.name}</label>
                        <span className="text-[10px] font-mono text-tactical-red">${p.price.toFixed(2)}</span>
                      </div>
                      <input 
                        type="text" 
                        value={customLinks[p.id] || ''}
                        onChange={(e) => updateLink(p.id, e.target.value)}
                        placeholder="https://checkout.square.site/buy/..."
                        className="w-full bg-tactical-black border border-white/10 rounded px-4 py-3 text-sm focus:outline-none focus:border-tactical-red transition-colors"
                      />
                    </div>
                  ))}
                </div>
              ...
              <div className="p-6 border-t border-white/10 flex justify-end bg-tactical-gray">
                <button 
                  onClick={() => setIsAdminOpen(false)}
              ...
              {cartCount === 0 ? (
                ...
                <div className="space-y-6">
                  <div className="flex items-center space-x-4 glass-panel p-4">
                    <img src={activeProduct.image} className="w-16 h-16 object-cover rounded" />
                    <div className="flex-1">
                      <h4 className="font-bold uppercase text-sm">{activeProduct.name}</h4>
                      <p className="text-xs opacity-50">${activeProduct.price.toFixed(2)}</p>
                    </div>
                  ...
                  <div className="pt-8 border-t border-white/10">
                    <div className="flex justify-between mb-4">
                      ...
                      <span className="font-mono text-xl">${(activeProduct.price * cartCount).toFixed(2)}</span>
                    </div>
